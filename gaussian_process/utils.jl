...
"solveKiW(K, W::Diagonal, r::Vector) = (K+inv(W)) \ r.  Here, W is specified by its diagonal."
function solveKiW(K::Symmetric, Wdiag::Vector, r::Vector)
    if any(Wdiag .< 0)
        return Wdiag .* (Q*r)
    else

        return solve_chol(L,bsxfun(@times,r,sW)) .* sW
    end
end # function

"""
function brentmin(xlow,xupp,Nitmax,tol,f,nout,varargin)

BRENTMIN: Brent's minimization method in one dimension


% code taken from
%    Â§ 10.2 Parabolic Interpolation and Brent's Method in One Dimension
