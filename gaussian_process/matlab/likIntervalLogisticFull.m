function [varargout] = likIntervalLogisticFull(aparams, hyp, y, mu, s2, infMethod, i)
% 
...
%
% This has one hyperparameter h, which together with aparams = [al, ah] gives a as:
%   a = al + (ah-al)/(1+exp(-h)) // note the logistic function
% Essentially we want a bounded between al and ah.
%
% da/dh = (ah-al)*(1/(1+exp(-h)) - 1/(1+exp(-h))^2)
%
...
al = aparams(1);  ah = aparams(2);  assert(al < ah && al > 0);
alogistic = logistic(hyp(1));
a = al + (ah-al)*alogistic;
dadh = (ah-al)*(alogistic - alogistic^2);
% a = exp(hyp);
% dadh = a;
% a = 15;
...
if nargin<6                              % prediction mode if inf is not present
    ...
    if s2zero                                         % log probability evaluation
        ...
        lp = likIntervalLogisticFull(aparams, hyp, y, mu, s2, 'infLaplace');
    end
    ymu = {}; ys2 = {};
    if nargout>1
        p = exp(lp);
    ...
    switch infMethod
        case 'infLaplace'
            if nargin<7                                             % no derivative mode
                ...
                % Add values for finite h's
                hfin = isfinite(h);
                hf = zeros(N,1);
                hf(hfin) = h(hfin)-f(hfin);
                dlph(hfin) = logistic(-a*hf(hfin));
                dlp(hfin) = dlph(hfin) .* hf(hfin);
                
                % Add values for finite l's
                lfin = isfinite(l);
                lf = zeros(N,1);
                lf(lfin) = l(lfin)-f(lfin);
                dlpl(lfin) = logistic(a*lf(lfin));
                dlp(lfin) = dlp(lfin) - dlpl(lfin) .* lf(lfin);
                
                % Add values for when both l and h are finite
                lhfin = hfin & lfin;
                hml = h(lhfin) - l(lhfin);
                dlp(lhfin) = dlp(lhfin) + hml ./ expm1(a*hml);
                
                d2lpl = dlpl - dlpl.^2;
                d2lph = dlph - dlph.^2;
                lfd2lpl = lf.*d2lpl;
                hfd2lph = hf.*d2lph;
                d2lp = dlpl - dlph + a*(lfd2lpl + hfd2lph);
                
                d3lp = -a^2 * ((1-2*dlpl).*lfd2lpl - (1-2*dlph).*hfd2lph) - 2*a*(d2lpl + d2lph);
                
                % Note that hyper = log(a), and derivatives are w.r.t.
                % hyper
                varargout = {dadh*dlp,dadh*d2lp,dadh*d3lp};                         % derivative w.r.t. hypers
            end
